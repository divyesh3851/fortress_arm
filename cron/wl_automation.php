...
if (!get_option('wl_email_cron')) {
    return;
}

$reminder_time = date('Y-m-d H:i', strtotime(current_time('mysql')));

$get_advisor_list = $wpdb->get_results("SELECT ad.id,ad.first_name,ad.last_name,ad.email,ad.gender,ad.birth_date,ad.state,ad.created_by,ad.created_by_type,interest.id as interest_id, interest.wl_mail_reminder FROM advisor as ad INNER JOIN interest ON ad.id = interest.advisor_id WHERE interest.life_insurance != '' AND FIND_IN_SET('3', interest.life_insurance) AND ad.advisor_status = 2 AND ad.status = 0 AND interest.wl_mail_reminder LIKE '" . $reminder_time . "%' AND ( interest.wl_mail_reminder != '' OR interest.wl_mail_reminder != NULL OR interest.wl_mail_reminder != '0000-00-00 00:00:00')");
...
foreach ($get_advisor_list as $advisor_result) {
    ...
    $wl_email_enable = Advisor()->get_advisor_meta($advisor_result->id, 'wl_step_' . $current_step . '_email_enable');
    ...
    $current_time   = strtotime($advisor_result->wl_mail_reminder);
    ...
    if ($current_step == 2) {
        ...
        if ($status) {
            ...
            $wpdb->update("interest", array("wl_mail_reminder" => $next_mail_reminder_date), array("id" => $advisor_result->interest_id));
        ...
        if ($status) {
            ...
            $wpdb->update("interest", array("wl_mail_reminder" => date('Y-m-d H:i:s', strtotime(current_time('mysql')))), array("id" => $advisor_result->interest_id));
