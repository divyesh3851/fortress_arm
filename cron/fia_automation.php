...
if (!get_option('fia_email_cron')) {
    return;
}

$reminder_time = date('Y-m-d H:i', strtotime(current_time('mysql')));

$get_advisor_list = $wpdb->get_results("SELECT ad.id,ad.first_name,ad.last_name,ad.email,ad.gender,ad.birth_date,ad.state,ad.created_by,ad.created_by_type,interest.id as interest_id, interest.fia_mail_reminder FROM advisor as ad INNER JOIN interest ON ad.id = interest.advisor_id WHERE interest.annuities != '' AND ad.advisor_status = 2 AND ad.status = 0 AND interest.fia_mail_reminder LIKE '" . $reminder_time . "%' AND ( interest.fia_mail_reminder != '' OR interest.fia_mail_reminder != NULL OR interest.fia_mail_reminder != '0000-00-00 00:00:00')");
...
foreach ($get_advisor_list as $advisor_result) {
    ...
    $fia_email_enable = Advisor()->get_advisor_meta($advisor_result->id, 'fia_step_' . $current_step . '_email_enable');
    ...
    $current_time   = strtotime($advisor_result->fia_mail_reminder);
    ...
    if ($status) {
        ...
        $wpdb->update("interest", array("fia_mail_reminder" => $next_mail_reminder_date), array("id" => $advisor_result->interest_id));
