...
$step_2_subject = get_option('iul_step_2_subject');
...
foreach ($get_advisor_list as $advisor_result) {
    ...
    $step_2_mail_body = get_option('iul_step_2_mail_body');

    $step_2_mail_body  = replace_merge_fields($step_2_mail_body, $merge_fields);

    $_SESSION['use_smtp'] = true;
    //send_mail($advisor_result->email, $step_2_subject, $step_2_mail_body);
    $status = send_mail('demo.starline2020@gmail.com', $step_2_subject, $step_2_mail_body);
    if ($status) {
        ...
        Admin()->create_mail_log($advisor_result->id, $advisor_result->email, 'advisor_interest_iul_step_2', 'advisor');
