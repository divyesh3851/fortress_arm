...
$check_user = $wpdb->get_row("SELECT id FROM admin WHERE reset_password_key = '" . siget('key') . "' AND status = 0 ");
...
if (isset($_POST['update_password'])) {
    ...
    if (!sipost('password') || !sipost('confirm-password')) {
        ...
        wp_redirect(site_url() . '/admin/reset-password/' . siget('key'));
        die();
    }
    ...
    $response = Admin()->update_password(siget('key'));
