...
$totalRecords = $wpdb->get_var('SELECT COUNT( * ) FROM premium_volume WHERE status = 0');

$totalRecordwithFilter = $wpdb->get_var('SELECT COUNT( * ) FROM premium_volume WHERE status = 0');

$premium_volume_list   = $wpdb->get_results('SELECT * FROM premium_volume WHERE status = 0 ORDER BY id DESC');

$i = 1;
foreach ($premium_volume_list as $result) {
